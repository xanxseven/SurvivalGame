import { ARRAY_TYPE as gl_ARRAY_TYPE, RANDOM as gl_RANDOM, EPSILON as gl_EPSILON } from "./common";
...
export function create(): vec2 {
    let out = new gl_ARRAY_TYPE(2);
    if (gl_ARRAY_TYPE != Float32Array) {
        out[0] = 0;
        out[1] = 0;
    }
    return out;
 ...
 * @returns {vec3} out
 */
export function cross(out: vec2, a: vec2, b: vec2): any {
 ...
 * @returns {vec3} out
 */
export function crossScalar(out: vec2, a: vec2, b: number): vec2 {
    out[0] = a[1] * b
    out[1] = -a[0] * b;
    return out;
}

export function cross1d(a: vec2, b: vec2): number {
...
export function equals(a, b) {
    ...
    return (
        Math.abs(a0 - b0) <=
        gl_EPSILON * Math.max(1.0, Math.abs(a0), Math.abs(b0)) &&
        Math.abs(a1 - b1) <=
        gl_EPSILON * Math.max(1.0, Math.abs(a1), Math.abs(b1))
    );
}
